...
- [ ] Passwords hashed with bcrypt/scrypt/argon2 (salt rounds ≥ 12); sessions are httpOnly, secure, sameSite
- [ ] Login rate-limited; password reset tokens expire
- [ ] Every endpoint checks user permissions; users can only access their own resources; admin actions verify the admin role

### Input & Data
- [ ] All user input validated at the boundary; SQL parameterized; HTML output encoded
- [ ] Server-side URL fetches allowlisted (no SSRF); delete/move/overwrite targets checked against an allowlisted root, minimum depth, and ownership evidence read before the operation
- [ ] No secrets in code or version control; sensitive fields excluded from API responses; PII encrypted at rest
- [ ] Personal data classified, minimized to a stated purpose, and has a retention limit with a working deletion path (incl. backups/indexes); export/delete requests supported where required; third-party sharing has consent

### Infrastructure & Supply Chain
- [ ] Security headers configured (CSP, HSTS); CORS restricted to known origins
- [ ] Dependencies audited; error messages don't expose internals
- [ ] One authoritative lockfile committed; CI uses frozen/immutable install; audit findings triaged by reachability; install scripts blocked unless explicitly approved; new dependencies reviewed (ownership, provenance, release age, transitive graph)
...
- No rate limiting on authentication endpoints, or an in-memory limiter in front of more than one instance; stack traces or internal errors exposed to users
...
- LLM/model output passed into a query, the DOM, a shell, or `eval`; secrets, PII, or the full system prompt placed inside an LLM context window
- Personal data collected with no stated purpose, retention limit, or deletion path; PII sent to analytics/ad/LLM vendors with no consent or data-processing agreement
...
- [ ] Error responses don't expose internal details; rate limiting active on auth endpoints, backed by a shared store when more than one instance serves traffic
